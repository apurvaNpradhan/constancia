...
export default function Home() {
   const healthCheck = useQuery(trpc.testRouter.healthCheck.queryOptions());
   const privateData = useQuery(trpc.testRouter.privateData.queryOptions());
         ...
         <ScrollView className="flex-1">
            <View className="px-4">
               ...
               <View className="mb-6 rounded-lg border border-border p-4">
                  ...
                  {privateData && (
                     <View>
                        <Text className="text-muted-foreground">{privateData.data?.message}</Text>
                     </View>
                  )}
               </View>
               {!session?.user && (
